...
export default function ADProductUpdateForm(props) {
  const {
    id: idProp,
    aDProduct: aDProductModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = aDProductRecord
      ? { ...initialValues, ...aDProductRecord }
      : initialValues;
    setName(cleanValues.name);
    setImages(cleanValues.images ?? []);
    setCurrentImagesValue("");
    setPaths(cleanValues.paths ?? []);
    setCurrentPathsValue("");
    setDescription(cleanValues.description);
    setSuggestedPrice(cleanValues.suggestedPrice);
    setCreatedBy(cleanValues.createdBy);
    setErrors({});
  };
  const [aDProductRecord, setADProductRecord] =
    React.useState(aDProductModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(ADProduct, idProp)
        : aDProductModelProp;
      setADProductRecord(record);
    };
    queryData();
  }, [idProp, aDProductModelProp]);
  React.useEffect(resetStateValues, [aDProductRecord]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            ADProduct.copyOf(aDProductRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "ADProductUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || aDProductModelProp)}
